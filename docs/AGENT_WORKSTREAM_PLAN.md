...
| **M QA** | Promote `test/CaptureSlice.tscn` into the evidence workflow; add a render-smoke + no-error gate; document the windowed-capture command. | `test/CaptureSlice.gd`, CI/docs | Every PR can attach a fresh screenshot bundle + GUT result. |
...
as a high-quality lead list, not ground truth. The same windowed-capture + GUT discipline that found
the New Game hang applies to every claim before you build on it.
