...
|---|---|---|---|---|---|---|
| R1 | **No authored gameplay art.** Every actor is a colored circle; spec DoD "zero hero primitives" is 0% met. Art is the long-pole and the #1 "looks like a prototype" cause. | 5 | 5 | **25** | Commit the Nightglass direction (already validated by existing concept art). Pipeline: generate top-down framed sprites → chroma-key magenta → atlas → `SpriteFrames`. Start with player + 4 slice actors only. Use existing JPGs as menu/portrait art now. | H (Art) |
